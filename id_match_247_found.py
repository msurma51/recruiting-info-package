...
import numpy as np
import os
from match_247_func import height_fix, comp_score_247
from namespace import league_path


match_file = os.path.join(league_path,'unmatched_247.csv')
search_file = os.path.join(league_path, 'found_247.csv')

# Initialize dataframes from filename inputs
df_search = pd.read_csv(search_file, index_col = 0)
df_search['data.height'] = df_search['data.height'].map(height_fix)
...
if 'no_match_247' not in df_match.columns:
    df_match['no_match_247'] = False
df_unmatched = df_match.query('no_match_247 == False')
...
for index, row in df_unmatched.iterrows():
    try:
        ...
        print(e)
        df_match.at[index,'no_match_247'] = True
        if max_score > 0.0:    
